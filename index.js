...
// ✅ CORREÇÃO DE CORS PARA PERMITIR OS DOMÍNIOS DO CLIENTE
...
const corsOptions = {
  origin: function (origin, callback) {
    // Permite requisições sem 'origin' (Postman, etc) ou se a origem estiver na lista
...
// Rota para criar a preferência de pagamento (agora mais inteligente)
app.post('/api/criar-checkout', async (req, res) => {
  ...
  try {
    ...
    // Se o valor do frete for passado pela página intermediária, usa esse valor.
    // Senão, calcula na hora (mantendo a compatibilidade com o fluxo antigo, se necessário).
    let freteFinal;
    if (valorFrete) {
      ...
      freteFinal = opcoesDeFrete[0]; // Pega a primeira opção como padrão
